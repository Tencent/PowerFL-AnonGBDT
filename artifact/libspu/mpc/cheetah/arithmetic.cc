  ...
  return DISPATCH_ALL_FIELDS(field, "_", [&]() {
    ...
    auto carry_bit = TiledDispatchOTFunc(
                         ...
                         [&](const NdArrayRef& input,
                           ...
                           return prot.Compute(input, /*greater*/ true, nbits);
                         })
                         .as(x.eltype());
...
NdArrayRef MsbA2BSigmoid::proc(KernelEvalContext* ctx,
  ...
  auto *ot_state = ctx->getState<cheetah::CheetahOTState>();
  ...
  return DISPATCH_ALL_FIELDS(field, "_", [&]() {
    ...
    auto carry_bit_shape = x.shape();
    carry_bit_shape.at(0) = carry_bit_shape.at(0) * 2;
    NdArrayRef carry_bit(x.eltype(), carry_bit_shape);
    NdArrayView<u2k> _carry_bit(carry_bit);

    CompareSigmoidProtocol prot(ctx->getState<CheetahOTState>()->get(0));
    if (rank == 0) {
      NdArrayRef adjusted_a = ring_zeros(field, {numel});
      NdArrayRef adjusted_b = ring_zeros(field, {numel});
      auto xadj_a = NdArrayView<u2k>(adjusted_a);
      auto xadj_b = NdArrayView<u2k>(adjusted_b);
      // x0 - a, x0 - b
      pforeach(0, numel,
               [&](int64_t i) { xadj_a[i] = (xinp[i] - low_seg_) & mask; });
      pforeach(0, numel,
               [&](int64_t i) { xadj_b[i] = (xinp[i] - high_seg_) & mask; });
      auto [res_a, res_b] =
          prot.Compute(adjusted_a, adjusted_b, 1, nbits, skip_bits_);

      // get carray bit
      auto xres_a = NdArrayView<const u2k>(res_a);
      auto xres_b = NdArrayView<const u2k>(res_b);

      // [msb(x-a)]_B <- [1{x0 + x1 -a > 2^{k- 1} - 1]_B ^ msb(x0-a)
      pforeach(0, numel, [&](int64_t i) {
        _carry_bit[i] = xres_a[i] ^ ((xinp[i] - low_seg_) >> shft);
      });
      pforeach(0, numel, [&](int64_t i) {
        _carry_bit[i + numel] = (xres_b[i] ^ ((xinp[i] - high_seg_) >> shft));
      });
    } else {
      ...
      // 2^{k - 1} - 1 - x1
      pforeach(0, numel, [&](int64_t i) { xadj[i] = (mask - xinp[i]) & mask; });
      auto [res_a, res_b] = prot.Compute(adjusted, 1, nbits, skip_bits_);

      // get carray bit
      auto xres_a = NdArrayView<const u2k>(res_a);
      auto xres_b = NdArrayView<const u2k>(res_b);
      // [msb(x-a)]_B <- [1{x0 + x1 -a > 2^{k- 1} - 1]_B ^ msb(x1)
      pforeach(0, numel, [&](int64_t i) {
        _carry_bit[i] = xres_a[i] ^ (xinp[i] >> shft);
      });
      pforeach(0, numel, [&](int64_t i) {
        _carry_bit[i + numel] = xres_b[i] ^ (xinp[i] >> shft);
      });
    }
...
NdArrayRef MulAA::mulDirectly(KernelEvalContext* ctx, const NdArrayRef& x,
  ...
  auto ff = ring_zeros(field, {x.numel() + y.numel()}).as(x.eltype());

  if (rank == 0) {
    ff.update_slice(x.reshape({x.numel()}), {0});
    ff.update_slice(y.reshape({y.numel()}), {x.numel()});
  } else {
    ff.update_slice(y.reshape({y.numel()}), {0});
    ff.update_slice(x.reshape({x.numel()}), {y.numel()});
  }

  if (rank == 0) {
    ff = mul_prot->MulOLE(ff, true);
  } else {
    ff = mul_prot->MulOLE(ff, false);
  }

  auto aa = ff.slice({0}, {x.numel()}, {1});
  auto bb = ff.slice({x.numel()}, {ff.numel()}, {1});
  return ring_add(ring_add(aa, bb).reshape(x.shape()), ring_mul(x, y))
      .as(x.eltype());
}
