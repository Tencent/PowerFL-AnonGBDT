...
//
// Licensed under the Apache License, Veion 2.0 (the "License");
...
static Value rsqrt_init_guess(SPUContext* ctx, const Value& x, const Value& z) {
  ...
  // from PowerFL: using sqrt(u) = 2.9390912778777811 * u^4 -5.526333964374774*u^3, 6.908849226372312+u^2 -4.5026771451995931 *u+ 1.1811597257071498 when converting u to [0.5,1)
  ...
  if (!ctx->config().enable_lower_accuracy_rsqrt()) {
    ...
    auto coeffs = {0.0F, -5.526333964374774F, 6.908849226372312F, -4.5026771451995931F, 1.1811597257071498F};
    r = f_add(ctx, detail::polynomial(ctx, u, coeffs),
              constant(ctx, 2.9390912778777811F, x.dtype(), x.shape()));
  } else {
    // auto coeffs = {0.0F, -5.9417F, 4.7979F};
    // r = f_add(ctx, detail::polynomial(ctx, u, coeffs),
    //           constant(ctx, 3.1855F, x.dtype(), x.shape()));
    auto coeffs = {0.0F, -2.0978571553217327F, 0.85650928091739453F};
    r = f_add(ctx, detail::polynomial(ctx, u, coeffs),
              constant(ctx, 2.2451813471044755F, x.dtype(), x.shape()));
  }
