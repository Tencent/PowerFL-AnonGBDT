...
#include "libspu/mpc/cheetah/pir/pir.h"
...
void BindLibs(py::module& m) {
  ...
  m.def(
          ...
          [](const std::string& role,
             const std::shared_ptr<yacl::link::Context>& lctx,
             const std::vector<std::string>& items) -> std::tuple<std::vector<std::vector<int>>, py::bytes> {
                std::string lower_role = role;
                for (char &c : lower_role) {
                    c = static_cast<char>(std::tolower(c));
                }

                psi::PsiRoleType psi_role = psi::PsiRoleType::Receiver;
                if (lower_role != "guest" && lower_role != "sender") {
                    psi_role = psi::PsiRoleType::Sender;
                }

                auto ff = spu::mpc::cheetah::VoleCPsi(lctx, psi_role);
                auto rr = ff.RunCPsi(absl::MakeSpan(items));

              std::vector<std::vector<int>> bin_index(ff.getNumBin());
              for (size_t i = 0; i < bin_index.size(); i++) {
                  auto res = ff.getItemIndex(i);
                  bin_index[i].resize(res.size());
                  for (size_t j = 0; j < res.size(); j++) {
                      bin_index[i][j] = static_cast<int>(res[j]);
                  }
              }

              return std::make_tuple(bin_index, py::bytes(reinterpret_cast<const char*>(rr.data()), rr.size()));
    ...
    m.def(
            ...
            [](const std::shared_ptr<yacl::link::Context>& lctx,
               const std::string& item) -> std::string {
                ...
                auto proc = spu::mpc::cheetah::PerceptPIR(lctx, psi::PsiRoleType::Sender, 1);
//
//                size_t bucket_size = 0;
//                std::string res = proc.parallel_run_client(item, bucket_size);
//                return res;
                return "";
...
//               size_t bucket_size, size_t num_thread) -> std::string {
//
//                spu::mpc::cheetah::PerceptPIR proc(lctx, psi::PsiRoleType::Receiver, num_thread);
//
//                std::string res = proc.parallel_run_server(key_values, bucket_size);
//                return res;
//
//            }, NO_GIL);
}
