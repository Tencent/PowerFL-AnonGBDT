...
llvm::cl::opt<uint32_t> BucketSize("bucket_size", llvm::cl::init(16),
                                   llvm::cl::desc("bucket size"));

llvm::cl::opt<uint32_t> MaxDepth("max_depth", llvm::cl::init(5),
                                 llvm::cl::desc("max_depth"));

llvm::cl::opt<uint32_t> NumTrees("num_tree", llvm::cl::init(1),
...
llvm::cl::opt<bool> EngineTrace("engine_trace", llvm::cl::init(true),
...
int main(int argc, char** argv) {
  ...
  auto time_diff = std::chrono::duration_cast<std::chrono::microseconds>(
      time_end - time_start);
  std::cout << "total time: " << time_diff.count() << " us ";
  hctx->lctx()->PrintStats();
  // Test on train set
